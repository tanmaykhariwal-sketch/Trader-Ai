...
export const CumulativePnLChart: React.FC<CumulativePnLChartProps> = ({
  ...
  const { chartData, metrics } = useMemo(() => {
    ...
    // 'ALL' means every entry regardless of age — it previously silently
    // capped at 90 days, the same limit as no timeframe selection at all.
    // Since the day-by-day curve below needs a finite number of days to
    // iterate, 'ALL' spans from the earliest entry to today instead.
    const daysLimit = timeframe === '7D' ? 7 : timeframe === '14D' ? 14 : timeframe === '30D' ? 30
      : sortedEntries.length > 0
        ? Math.max(1, Math.ceil((now.getTime() - sortedEntries[0].parsedDate.getTime()) / (24 * 60 * 60 * 1000)))
        : 0;
    ...
    // The window's curve intentionally starts from a 0 baseline (growth
    // within the selected timeframe only, not lifetime-carried) — this is
    // the only cumulative total actually used below.
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-5">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-base font-extrabold text-white tracking-tight">
                {timeframe === '30D' ? '30-Day' : timeframe === 'ALL' ? 'All-Time' : timeframe} Cumulative P&L Growth
              </h2>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Visualizes the compound growth and net returns of closed journal positions
              {timeframe === 'ALL' ? ' across your full trading history.' : ` over the last ${timeframe.replace('D', ' days')}.`}
            </p>
          </div>
        </div>
      ...
      {/* Chart Footer with Trade Insights. The reduce() that finds
          bestTrade has no positive-value floor, so when every closed trade
          in the window lost money it returns the LEAST-bad loss — this
          footer used to hardcode a "+" prefix and emerald styling
          regardless, rendering e.g. "+₹-820.00" in green for a losing
          trade. Now reflects the trade's real sign, and relabels itself
          honestly when there's nothing to call a genuine win. */}
      {metrics.bestTrade && (() => {
        const isActuallyProfit = metrics.bestTrade.pnl >= 0;
        return (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-slate-950/80 p-3 rounded-xl border border-slate-800 text-xs">
            <div className="flex items-center space-x-2 text-slate-300">
              <Award className={`h-4 w-4 flex-shrink-0 ${isActuallyProfit ? 'text-emerald-400' : 'text-rose-400'}`} />
              <span>
                <strong className="text-white">{isActuallyProfit ? 'Best Trade in Window:' : 'Smallest Loss in Window:'}</strong> {metrics.bestTrade.stockName} ({metrics.bestTrade.symbol})
              </span>
            </div>
            <div className={`font-mono font-bold flex items-center space-x-1.5 ${isActuallyProfit ? 'text-emerald-400' : 'text-rose-400'}`}>
              <span>{isActuallyProfit ? '+' : ''}{currSymbol}{metrics.bestTrade.pnl.toFixed(2)}</span>
              <span className={`text-[11px] px-1.5 py-0.5 rounded border ${
                isActuallyProfit ? 'text-emerald-300 bg-emerald-500/20 border-emerald-500/30' : 'text-rose-300 bg-rose-500/20 border-rose-500/30'
              }`}>
                {isActuallyProfit ? '+' : ''}{metrics.bestTrade.pnlPercent.toFixed(1)}% ROI
              </span>
            </div>
          </div>
        );
