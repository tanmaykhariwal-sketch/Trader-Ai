...
export const CapitalGrowthChart: React.FC<CapitalGrowthChartProps> = ({
  ...
  // `id` is a random UUID with no ordering meaning — sort by the real
  // timestamp instead, or every reload could shuffle the growth curve.
  const sortedRecords = [...capitalRecords].sort((a, b) => a.sortKey - b.sortKey);
  ...
  // Deliberately NOT `currentCapital - initialCap`: cash-on-hand naturally
  // dips the moment a holding is bought (PURCHASE_DEBIT) and rises on a sale
  // (SALE_CREDIT) — neither is money leaving or entering the account, it's
  // just capital changing form between cash and a position. "Net Added" is
  // real owner-contributed capital only: sum the actual applied delta from
  // DEPOSIT/SET records, ignoring trading-activity record types entirely.
  const netContributed = sortedRecords
    .filter(r => r.type === 'DEPOSIT' || r.type === 'SET')
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
                ...
                if (active && payload && payload.length) {
                  ...
                  return (
                    <div className="bg-slate-950 border border-slate-700 p-3 rounded-xl shadow-2xl space-y-1 font-mono text-xs">
                      ...
                      {/* Was gated on `delta > 0`, so hovering any downward
                          step (a withdrawal, a downward SET correction, a
                          PURCHASE_DEBIT) showed only the capital total with
                          no indication of what changed or by how much — the
                          record's own note text was hidden along with it. */}
                      {dataPoint.delta !== 0 && (
                        <div className={`text-[11px] ${dataPoint.delta > 0 ? 'text-cyan-400' : 'text-rose-400'}`}>
                          {dataPoint.delta > 0 ? '+' : '-'}{currSym}{Math.abs(dataPoint.delta).toLocaleString()} ({dataPoint.note})
                        </div>
                      )}
                    </div>
                  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs font-mono pt-1 text-slate-400 border-t border-slate-800/80">
        ...
        <div className="flex items-center space-x-3">
          <span>Net {netContributed >= 0 ? 'Added' : 'Withdrawn'}: <strong className={netContributed >= 0 ? 'text-emerald-400' : 'text-rose-400'}>{netContributed >= 0 ? '+' : '-'}{currSym}{Math.abs(netContributed).toLocaleString()}</strong></span>
