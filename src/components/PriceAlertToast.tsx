...
export const PriceAlertToast: React.FC<PriceAlertToastProps> = ({
  ...
  // Which alert ids have already had their chime played — the effect used
  // to key on `alerts.length`, so it fired on ANY length change including a
  // DECREASE (dismissing one alert re-triggered the chime for whatever was
  // left, using alerts[0]'s tone regardless of which alert was dismissed).
  // Tracking played ids means a chime only ever plays once, for a genuinely
  // new alert.
  const playedAlertIdsRef = useRef<Set<string>>(new Set());
  // Reused across every chime instead of `new AudioContext()` per alert —
  // Chrome enforces a hard cap on concurrent contexts (~6), and the old
  // per-chime instance was never closed; hitting the cap threw inside an
  // empty catch, silently killing all future alert sounds for the rest of
  // the session with no indication to the user.
  ...
  useEffect(() => {
    ...
    const newAlert = alerts.find(a => !playedAlertIdsRef.current.has(a.id));
    if (!newAlert) return;
    playedAlertIdsRef.current.add(newAlert.id);
    ...
    try {
      ...
      const isTarget = newAlert.alertType === 'TARGET_MET';
  ...
  return (
    <div
      role="status"
      aria-live="assertive"
      className="fixed top-20 right-4 sm:right-6 z-[110] max-w-sm sm:max-w-md w-full space-y-3 pointer-events-auto animate-in slide-in-from-top-4 fade-in duration-300"
    >
