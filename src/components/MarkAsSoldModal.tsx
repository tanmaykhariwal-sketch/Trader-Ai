...
interface MarkAsSoldModalProps {
  ...
  onConfirmSale: (entry: JournalEntry) => Promise<string | null>;
}
...
export const MarkAsSoldModal: React.FC<MarkAsSoldModalProps> = ({
  ...
  // String buffer, not a number: same clear-then-retype bug as
  // MarkAsBoughtModal's quantity field (parseInt(...) || 1 snapped an
  // emptied field to "1" and then prepended to the next keystroke).
  ...
  useEffect(() => {
    if (holding) {
      // Default to the holding's own real target price (the same number
      // shown right above in the Target Sell Zone banner) instead of an
      // arbitrary 2% guess that could contradict it.
      setSellPrice(computeHoldingMetrics(holding, null).target1);
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    e.preventDefault();
    if (sellPrice <= 0 || quantity <= 0) return;

    // Defensive clamp: the input's max={holding.quantity} stops normal use,
    // but never trust the DOM alone for a money-affecting quantity.
    const clampedQuantity = Math.min(quantity, holding.quantity);
    ...
    const newJournalEntry: JournalEntry = {
      ...
      pnl: Math.round((sellPrice - holding.purchasePrice) * clampedQuantity * 100) / 100,
    ...
    setSubmitError(null);
    // Previously fired-and-forgot: closed unconditionally regardless of
    // whether the server actually recorded the sale. Reachable in practice
    // via a stale session, a second tab having already sold/removed the
    // same holding (404), or a dropped connection — all of which used to
    // look identical to a successful sale.
    const errorMessage = await onConfirmSale(newJournalEntry);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl relative space-y-5">
        ...
        <form onSubmit={handleConfirm} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onBlur={() => {
                ...
                if (!n || n <= 0) setQuantityInput('1');
                else if (n > holding.quantity) setQuantityInput(String(holding.quantity));
              }}
          ...
          <div className="flex items-center space-x-3 pt-2">
            ...
            <button
              type="submit"
              disabled={sellPrice <= 0 || quantity <= 0 || isSubmitting}
              className="w-1/2 py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 disabled:bg-slate-700 disabled:text-slate-400 disabled:cursor-not-allowed text-slate-950 text-xs font-extrabold flex items-center justify-center space-x-1.5 shadow-lg shadow-cyan-500/20 transition-all"
            >
              <Check className="h-4 w-4" />
              <span>{isSubmitting ? 'Saving…' : 'Log Exit to Journal'}</span>
            </button>
          </div>
